...
describe('sweep', () => {
  ...
  it('mantiene una partida inactiva mientras haya alguien conectado y la cierra al quedar vacía', () => {
    ...
    // Al apagarse/bloquearse el último móvil empieza un margen completo:
    // la antigüedad previa de la partida no debe cerrarla inmediatamente.
    expect(m.sweep(NOW + 15 * 60_000)).toBe(0);
    expect(m.sweep(NOW + 30 * 60_000 - 1)).toBe(0);
    expect(m.sweep(NOW + 30 * 60_000)).toBe(1);
    expect(m.getRoomByCode(c.value.roomCode)).toBeUndefined();
  });
  ...
  it('da el margen completo desde que se bloquea el último móvil en una sala antigua', () => {
    ...
    expect(m.sweep(NOW + 60 * 60_000)).toBe(0);
    expect(m.sweep(NOW + 75 * 60_000 - 1)).toBe(0);
    expect(m.sweep(NOW + 75 * 60_000)).toBe(1);
  });
