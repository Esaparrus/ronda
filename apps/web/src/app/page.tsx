...
import { listSavedRooms } from '@/lib/token';
        ...
        <div className="flex w-full max-w-xs flex-col gap-3">
          {savedRooms.map((code) => (
            <Link
              key={code}
              href={`/sala/${code}`}
              className="flex min-h-14 items-center justify-center rounded-lg border border-linea bg-mesa px-6 text-16 font-semibold text-hueso"
            >
