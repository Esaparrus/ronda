      ...
      <main className="flex min-h-0 flex-1 flex-col items-center gap-5 overflow-y-auto px-4 py-6">
        <section className="surface-panel w-full max-w-md p-5 text-center">
          ...
          <p className="mt-3 text-16 font-semibold text-oro">
            Elige exactamente {party.answerCount}{' '}
            {party.answerCount === 1 ? 'color' : 'colores'}
          </p>
              ...
              return (
                ...
                >
                  <span className="flex flex-col items-center gap-1">
                    <span className="text-20 leading-none" aria-hidden="true">{color.symbol}</span>
    ...
    <section className="surface-panel flex w-full max-w-md flex-col gap-4 p-4">
      <div className="text-center">
        ...
        <div className="mt-2 flex flex-wrap justify-center gap-2">
          {correctColors?.map((color) => <ColorChip key={color} color={color} />)}
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <span className={correct ? 'text-14 font-semibold text-crema' : 'text-14 text-humo'}>
                  {correct ? (delta > 0 ? `+${delta}` : 'Acierto') : 'Fallo'}
                </span>
              </div>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {colors?.length ? colors.map((color) => <ColorChip key={color} color={color} />) : (
